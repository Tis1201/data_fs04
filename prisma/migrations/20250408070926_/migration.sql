/*
  Warnings:

  - You are about to alter the column `data` on the `WhatsAppAuthData` table. The data in that column could be lost. The data in that column will be cast from `String` to `Binary`.

*/
-- RedefineTables
PRAGMA defer_foreign_keys=ON;
PRAGMA foreign_keys=OFF;
CREATE TABLE "new_WhatsAppAuthData" (
    "id" TEXT NOT NULL PRIMARY KEY,
    "clientId" TEXT NOT NULL,
    "keyId" TEXT NOT NULL,
    "type" TEXT NOT NULL,
    "data" BLOB NOT NULL,
    "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" DATETIME NOT NULL
);
INSERT INTO "new_WhatsAppAuthData" ("clientId", "createdAt", "data", "id", "keyId", "type", "updatedAt") SELECT "clientId", "createdAt", "data", "id", "keyId", "type", "updatedAt" FROM "WhatsAppAuthData";
DROP TABLE "WhatsAppAuthData";
ALTER TABLE "new_WhatsAppAuthData" RENAME TO "WhatsAppAuthData";
CREATE INDEX "WhatsAppAuthData_type_idx" ON "WhatsAppAuthData"("type");
CREATE INDEX "WhatsAppAuthData_clientId_idx" ON "WhatsAppAuthData"("clientId");
CREATE UNIQUE INDEX "WhatsAppAuthData_clientId_keyId_key" ON "WhatsAppAuthData"("clientId", "keyId");
PRAGMA foreign_keys=ON;
PRAGMA defer_foreign_keys=OFF;
