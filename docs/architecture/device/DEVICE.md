...
4. **Ongoing Communication**
    ...
    - Device information can be updated by sending a POST request to `/api/device/add` with the API key in the header
...
- **Initial Phase**:
    ...
    - **Endpoint**: `/api/device/listen`
    - **Authentication**: `X-API-Key` header
    - **Example Request**:
      ```bash
      curl -i -N \
        -H "X-API-Key: <DEVICE_API_KEY>" \
        http://localhost:5173/api/device/listen
      ```
    - **Security**:
        - All requests must include a valid device API key
        - API keys are validated on every connect
        - Device can request short-lived JWTs for resource downloads via `/api/device/jwt`
