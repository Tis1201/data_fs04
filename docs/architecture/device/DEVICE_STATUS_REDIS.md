...
```
                               ┌──────────────────────────────────────────────────┐
                               │                Device Fleet                      │
                               │  (progress, complete, failure events)           │
                               └───────────────┬──────────────────────────────────┘
                                               │
                             (A) Ingest (poll) │
                                               ▼
                 ┌─────────────────────────────────────────────────────────────┐
                 │            ClickHouse (production) or File (dev)           │
                 │    queryClickHouseEvents() / startFileBasedPoller()        │
                 └───────────────┬─────────────────────────────────────────────┘
                                 │  (B) Scheduler tick with lock
                                 ▼
                 ┌─────────────────────────────────────────────────────────────┐
                 │        BundleStatusScheduler (or FileBasedPoller)          │
                 │  - distributedLockManager (if Redis)                       │
                 │  - initializeStateManager()                                │
                 │  - pollClickHouseWithLock()                                │
                 │  - processEventsWithStateValidation()                      │
                 │  - applyTimeouts()                                         │
                 │  - checkAndTransitionBundleState()                         │
                 │  - cleanupCompletedBundles()                                │
                 └───────────────┬─────────────────────────────────────────────┘
                                 │  (C) per-bundle batching and validation
                                 ▼
                 ┌─────────────────────────────────────────────────────────────┐
                 │               BundleEventProcessor (batch)                 │
                 │  - deduplicate (eventDeduplication)                        │
                 │  - upsert per-device progress                              │
                 │  - recompute wave aggregates                                │
                 │  - compute bundle aggregates                                │
                 │  - publish SSE/Pushpin:                                     │
                 │      subscription:bundle:wave:{waveId}                      │
                 │      subscription:bundle:{bundleId}                         │
                 └───────────────┬─────────────────────────────────────────────┘
                                 │  (D) state store (for processable bundles)
                                 ▼
                 ┌─────────────────────────────────────────────────────────────┐
                 │                 StateManager (Redis/File)                   │
                 │  - cleanupExpiredStates()                                   │
                 │  - getProcessableBundles()                                   │
                 │  - cache for timeouts / transitions                         │
                 └───────────────┬─────────────────────────────────────────────┘
                                 │  (E) real-time delivery
                                 ▼
                 ┌─────────────────────────────────────────────────────────────┐
                 │            SSE / Pushpin → UI (Admin/User)                  │
                 │  - Waves table, Bundle detail, Device detail                │
                 │  - Action logs / progress                                   │
                 └─────────────────────────────────────────────────────────────┘
```

Key points confirmed from code:
- Scheduler interval: `FILE_STATUS_POLL_MS` (default 10000ms) in `bundleStatusScheduler.ts`.
- ClickHouse preferred when `USE_CLICKHOUSE=true` or `CLICKHOUSE_URL` present; otherwise file poller.
- Distributed locking via Redis when available (`distributedLockManager.initialize`).
- Timeout checks always run (every 30–60s depending on path) via `applyTimeouts()` and cache sync on start.
- Batch processing and SSE publishing implemented in `bundleEventProcessor.ts`.
