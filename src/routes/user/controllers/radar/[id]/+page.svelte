...
<UserPageLayout
    ...
    actionButtons={[
        ...
        },
        {
            label: "Back",
            icon: ArrowLeft,
            onClick: () => goto("/user/controllers/radar"),
            variant: "outline",
        },
    ]}
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                        ...
                        {#if data.radarSensor.controller?.device}
                            <div>
                                ...
                                <div class="font-medium">
                                    {data.radarSensor.controller.device.name}
                                </div>
                            </div>
                        {/if}
                    </div>
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-2">
                                ...
                                <span class="text-sm font-medium"
                                    >Single Integrated Sensor</span
                                >
                            </div>
                            <div class="text-xs text-muted-foreground">
                                This radar controller includes one built-in
                                sensor for area monitoring
                            </div>
                        </div>
                        <Button
                            variant="outline"
                        ...
                        >
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="text-xs text-muted-foreground">
                                        {config?.dwellBuckets?.length || 0} time buckets
                                        configured
                                    </div>
                                </div>
                            </div>
        ...
        {#if data.radarSensor.controller?.device?.id}
            <Card>
                ...
                <CardContent class="pt-0 px-4 pb-4">
                    <div class="space-y-4">
                        <div class="text-sm text-muted-foreground">
                            View real-time radar data from this sensor. Requires
                            the device to be online and connected.
                        </div>
                        <RadarPreview
            ...
            <Card>
                ...
                <CardContent class="pt-0 px-4 pb-4">
                    <div class="p-4 text-center text-muted-foreground">
                        ...
                        <p class="text-sm">
                            No device linked to this controller.
                        </p>
                        <p class="text-xs">
                            Link a device to enable live sensor preview.
                        </p>
                    </div>
                </CardContent>
            </Card>
...
<Dialog bind:open={showControllerEditDialog}>
    <DialogContent class="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
            ...
            <DialogDescription
                >Update the radar controller details</DialogDescription
            >
        </DialogHeader>
...
<RadarSensorConfigDialog
    ...
    onDeleteDwellBucket={(bucketId, bucketName) => {
        if (
            !confirm(
                `Are you sure you want to delete dwell bucket "${bucketName}"?`,
            )
        )
            return;
        const formData = new FormData();
