<script lang="ts">
    ...
    $: activeTab = $page.url.searchParams.get("tab") || "account";
    let editMode = {
        account: false,
        notifications: false,
    };
    ...
    const {
        ...
        tainted: accountTainted,
    } = accountForm;
    const {
        form: notificationData,
        enhance: notificationEnhance,
    ...
    $: hasChanges = Boolean(
        $accountTainted && Object.keys($accountTainted).length > 0,
    );
    ...
    $: if (
        $accountData.name &&
        (!$accountData.slug ||
            $accountData.slug === generateSlug(previousName || ""))
    ) {
            ...
            >
                <svelte:fragment slot="actions">
                    ...
                    {:else}
                        ...
                        >
                            <Lock class="h-4 w-4" />
                            <span
                                >Only account admins can edit these settings</span
                            >
                        </div>
                    {/if}
                </svelte:fragment>
                ...
                {#if editMode.account}
                    ...
                    >
                        ...
                        <div
                            class="sticky bottom-0 bg-background border-t border-border p-4 -mx-4 -mb-4"
                        >
                    ...
                    <div class="space-y-6">
                        <div class="space-y-4">
                            ...
                            <p class="text-sm font-medium py-2">
                                {data.account?.name || "No account set"}
                            </p>
                        </div>
                    ...
                    {#if editMode.notifications}
                        <div class="flex space-x-2">
                            <Button
                                variant="outline"
                                size="sm"
                                on:click={() => toggleEdit("notifications")}
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between">
                                    ...
                                    <Switch
                                        name="email"
                                        checked={editMode.notifications
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <Switch
                                        name="security"
                                        checked={editMode.notifications
                                            ? $notificationData.security
                                            : data.settings?.securityAlerts ||
                                              false}
                                        disabled={!editMode.notifications}
                                        onCheckedChange={(checked) => {
                                            if (editMode.notifications) {
                ...
                <div class="space-y-6">
                    <div class="space-y-4">
                        ...
                        >
                            ...
                            <Button
                                variant={data.settings?.twoFactorEnabled
                                    ? "outline"
                                    : "default"}
                                size="sm"
                                disabled
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            ...
                            <p class="text-sm text-muted-foreground">
                                Last login: <RelativeDate
                                    date={data.user?.lastLogin || new Date()}
                                    format="relative"
                                />
                            </p>
                        </div>
                        <div class="space-y-2">
                            {#each data.activeSessions || [] as session}
                                ...
                                >
                                    <div class="flex items-center">
                                        <div
                                            class="p-2 rounded-full bg-muted mr-3"
                                        >
                                            <Key class="h-4 w-4" />
                                        </div>
                                        <div>
                                            ...
                                            <div
                                                class="flex items-center text-xs text-muted-foreground mb-1"
                                            >
