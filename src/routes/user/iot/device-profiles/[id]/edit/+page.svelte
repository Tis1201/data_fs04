<script lang="ts">
    ...
    const pageCrumbs: [string, string][] = [
        ...
        [data.profile.name, `/user/iot/device-profiles/${data.profile.id}`],
    ...
    const { form, errors, enhance, submitting, constraints, errorMessage } = createFormHandler(data.form, {
        successRedirect: `/user/iot/device-profiles/${data.profile.id}`,
    ...
    async function loadAssignedDevices() {
        try {
            ...
            const response = await fetch(`/api/user/iot/device-profiles/${data.profile.id}/devices?${params}`);
            if (response.ok) {
    ...
    async function handleAssign() {
        ...
        try {
            ...
            // Handle assignments
            const response = await fetch(`/api/user/iot/device-profiles/${data.profile.id}/assign`, {
                method: 'POST',
    ...
    async function handleUnassign(deviceId: string) {
        try {
            const response = await fetch(`/api/user/iot/device-profiles/${data.profile.id}/unassign`, {
                method: 'POST',
                headers: {
    ...
    async function handleUnassignAll() {
        ...
        deviceLoading = true;
        try {
            const response = await fetch(`/api/user/iot/device-profiles/${data.profile.id}/unassign-all`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' }
            });
    ...
    async function handleAddAll() {
        ...
        try {
            ...
            const response = await fetch(`/api/user/iot/device-profiles/${data.profile.id}/assign-all`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async function loadAllTags() {
        try {
            const response = await fetch(`/api/user/iot/devices/tags`);
            const result = await response.json();
            if (result.success) {
                allTags = result.tags;
            }
    ...
    async function loadTagDevices(tagId: string) {
        ...
        try {
            const response = await fetch(`/api/user/iot/device-profiles/${data.profile.id}/devices-by-tag?tagId=${tagId}`);
            const result = await response.json();
            if (result.success) {
    ...
    async function handleAssignByTag() {
        ...
        deviceLoading = true;
        try {
            const response = await fetch(`/api/user/iot/device-profiles/${data.profile.id}/assign-by-tag`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    actionButtons={[
        {
            label: "Cancel",
            icon: ArrowLeft,
            onClick: () => goto(`/user/iot/device-profiles/${data.profile.id}`),
            variant: "outline"
        },
        {
            label: "Save Changes",
