<script lang="ts">
  ...
  const pageCrumbs: [string, string][] = [
    ...
    [preclaimSet.name || "Pre-claim Set", `/admin/iot/preclaims/${preclaimSet.id}`],
    ["Edit", ""]
  ];

  const { form, errors, enhance, submitting, errorMessage } = createFormHandler(data.form, {
    successRedirect: `/admin/iot/preclaims/${preclaimSet.id}`,
...
<PageContainer crumbs={pageCrumbs}>
  <PageHeader title={title}>
    <div slot="action" class="flex items-center space-x-2">
      ...
      <Button variant="outline" on:click={() => goto(`/admin/iot/preclaims/${preclaimSet.id}`)} class="flex items-center">
