<script lang="ts">
    ...
    let formData = {
        ...
        targetType: data.rule.targetType,
    ...
    onMount(async () => {
        ...
        try {
            ...
            selectedApps = new Set<string>(formData.apps.split(',').map((s) => s.trim()).filter(Boolean));
        } catch (e) {
            availableApps = [];
        }
    });
    ...
    $: if (formData.targetType === 'specific') {
        const ids = (formData.targetValue || '').split(',').map((s) => s.trim()).filter(Boolean);
        (async () => {
            if (ids.length === 0) { selectedDevices = new Map(); return; }
            try {
                const params = new URLSearchParams();
                params.append('includeDeviceIds', ids.join(','));
                const res = await fetch(`/api/admin/devices/select?${params.toString()}`);
                ...
                selectedDevices = new Map(ids.map((id) => [id, id]));
            }
        })();
    }
