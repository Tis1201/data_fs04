...
export const load = restrict(
  async (event: AuthenticatedLoadEvent) => {
    ...
    try {
      ...
      let allKeys = await listKeys(locals.prisma, 'TOKEN');
      
      // Get the primary key (active key) - we'll keep this separate from filtering
      const primaryKey = allKeys.find(key => key.isPrimary && key.keyType === 'TOKEN');
      ...
      if (sortField && sortOrder) {
        const field = sortField as keyof JwtSigningKey;
        keys = [...keys].sort((a, b) => {
  ...
  createKey: restrict(
    async (event: AuthenticatedEvent) => {
      ...
      try {
        ...
        if (!result.success) {
          logger.error('Failed to create factory key', { error: result.error });
    ...
    async (event: AuthenticatedEvent) => {
      ...
      try {
        ...
        if (existingKey.keyType !== 'TOKEN') {
          logger.error('Key is not a token key', { keyType: existingKey.keyType });
          return fail(400, {
            form,
            success: false,
            error: {
        ...
        if (!result.success) {
          logger.error('Failed to rotate factory key', { error: result.error });
          const errorData = result.error as any;
          return fail(400, {
            form,
            success: false,
