...
export const load = restrict(
  async (event: AuthenticatedLoadEvent) => {
    ...
    try {
      ...
      // Get all keys of type FACTORY
      let allKeys = await listKeys(locals.prisma, 'FACTORY');
      
      // Get the primary key (active key) - we'll keep this separate from filtering
      const primaryKey = allKeys.find(key => key.isPrimary && key.keyType === 'FACTORY');
      ...
      try {
        ...
        if (existingKey.keyType !== 'FACTORY') {
          ...
          return fail(400, {
            ...
            success: false,
            error: {
              message: `Selected key is not a factory key.`,
            },
          });
        }
        
        logger.info('Found existing factory key:', {
          id: existingKey.id,
  ...
  generateFactoryJwt: restrict(
    async (event: AuthenticatedEvent) => {
      ...
      try {
        ...
        if (!keyId) {
          return fail(400, { error: { message: 'keyId is required' } });
        }
        ...
        if (!key || !key.isActive || key.keyType !== 'FACTORY') {
          return fail(400, { error: { message: 'Invalid or inactive factory key' } });
        }
        ...
        // Sign with RS256 using private key; set kid header
