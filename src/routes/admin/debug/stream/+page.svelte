<script lang="ts">
        ...
        playPromise
          ...
          .catch((error) => {
            ...
            if (error.name === "NotAllowedError") {
              console.log(
                "[WebRTC] Autoplay prevented, waiting for user interaction",
              );
            } else if (error.name === "AbortError") {
              console.log(
                "[WebRTC] Play request was aborted, will retry in 1s",
              );
  ...
  $: if (
    $videoStream &&
    videoElement &&
    (!currentVideoStreamId || currentVideoStreamId !== $videoStream.id)
  ) {
  ...
  onMount(() => {
    ...
    const isH264Supported = tempVideoElement.canPlayType(
...
<div class="space-y-4 p-4">
  <Breadcrumb>
    <BreadcrumbList>
      <BreadcrumbItem>
        <a
          href="/admin"
          class="text-sm font-medium underline-offset-4 hover:underline">Main</a
        >
      </BreadcrumbItem>
      <BreadcrumbSeparator />
      <BreadcrumbItem>
      ...
      <div class="flex flex-col space-y-2">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <code class="px-1.5 py-0.5 rounded bg-gray-100 font-mono text-xs"
        ...
        {#if roomState.status?.password}
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <code class="px-1.5 py-0.5 rounded bg-gray-100 font-mono text-xs"
                >{roomState.status.password}</code
              >
            </div>
            <button
              class="text-gray-500 hover:text-gray-700"
              title="Copy Password"
              on:click={() =>
                navigator.clipboard.writeText(roomState.status.password)}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="14"
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          {#if roomState.status?.createdAt}
            <div>
              ...
              <span class="ml-1"
                >{new Date(roomState.status.createdAt).toLocaleString()}</span
              >
            </div>
          {/if}
          ...
          <div class="mt-2 p-2 bg-gray-50 rounded-md text-xs">
            {#if roomState.status?.admins && roomState.status.admins.length > 0}
              <div class="mb-2">
                ...
                {#each roomState.status.admins as admin, i}
                  <span class="font-mono">{admin}</span>{i <
                  roomState.status.admins.length - 1
                    ? ", "
                    : ""}
                {/each}
              </div>
                ...
                <ul class="ml-2 mt-1 space-y-1">
                  {#each roomState.status.participants as p}
                    <li class="flex items-center gap-2">
                      ...
                      {#if p.isAdmin}
                        <span
                          class="bg-yellow-100 text-yellow-800 px-1 rounded text-xs"
                          >admin</span
                        >
                      {/if}
                    </li>
            ...
            {#if roomState.status?.metadata}
              <div>
                ...
                <pre
                  class="bg-gray-100 p-2 rounded text-xs mt-1 overflow-x-auto">{JSON.stringify(
                    roomState.status.metadata,
                    null,
                    2,
                  )}</pre>
              </div>
  ...
  {:else if roomState?.error}
    <div
      class="bg-red-50 border border-red-200 rounded-md p-4 mb-4 text-red-700"
    >
      Error: {roomState.error}
    </div>
  {:else}
    <div
      class="bg-gray-50 border border-gray-200 rounded-md p-4 mb-4 text-gray-700"
    >
      Creating room...
    </div>
  {/if}
    ...
    <div class="relative aspect-video bg-black rounded-md overflow-hidden">
      {#if $videoStream}
        ...
        {#if isVideoPaused}
          ...
          >
            <div
              class="rounded-full bg-white bg-opacity-80 p-3 flex items-center justify-center"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
      ...
      <div class="flex items-center">
        ...
        {#if webrtcState?.connectionStatus === "connected"}
          <span class="text-green-600 font-medium flex items-center">
            <span class="inline-block w-2 h-2 bg-green-600 rounded-full mr-2"
            ></span>Connected
          </span>
        {:else if webrtcState?.connectionStatus === "disconnected"}
          <span class="text-red-600 font-medium flex items-center">
            <span class="inline-block w-2 h-2 bg-red-600 rounded-full mr-2"
            ></span>Disconnected
          </span>
        {:else}
          <span class="text-yellow-600 font-medium flex items-center">
            <span class="inline-block w-2 h-2 bg-yellow-600 rounded-full mr-2"
            ></span>Error: {webrtcState?.error}
          </span>
        {/if}
      </div>
      ...
      {#if webrtcState?.lastEventTimestamp}
        <div>
          ...
          <span class="text-gray-700 text-sm ml-1"
            >{new Date(webrtcState?.lastEventTimestamp).toLocaleString()}</span
          >
        </div>
      {/if}
      <div class="mt-4">
    ...
    {#if !webrtcState?.dataChannel || webrtcState?.dataChannelStatus !== "open"}
      <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-md">
        <p class="text-yellow-700">
          Data channel is not ready. Establish a WebRTC connection first.
        </p>
      </div>
    {:else}
      <div class="space-y-4">
        <div class="border border-gray-200 rounded-md p-2 h-64 overflow-y-auto">
          ...
          {:else}
            {#each webrtcState.dataChannelMessages as msg}
              <div
                class="mb-2 p-2 rounded-md {msg.direction === 'sent'
    ...
    {:else}
      <div class="space-y-3 max-h-[500px] overflow-y-auto pr-2">
        {#each filteredEvents as event}
          <div
            class="border border-gray-200 rounded-md p-3 {event.source ===
            'local'
              ? 'bg-blue-50'
              : ''}"
          >
            ...
            <div class="relative group mt-2">
              ...
              <div
                class="hidden group-hover:block absolute z-10 mt-1 w-full max-w-md bg-white border border-gray-200 rounded-md shadow-lg p-2"
              >
                <pre
                  class="bg-gray-50 p-2 rounded text-xs whitespace-pre-wrap break-all max-h-64 overflow-y-auto">
