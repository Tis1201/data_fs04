...
export const GET: RequestHandler = async ({ locals, setHeaders }) => {
  try {
    ...
    // According to Auth0 docs, the JWKS endpoint should have specific headers
    setHeaders({
    ...
    logger.debug('Serving factory JWKS for public consumption');
    ...
    // Check specifically for factory keys
    const factoryKeyCount = await locals.prisma.jwtSigningKey.count({
      where: {
        keyType: 'FACTORY'
      }
    });
    logger.debug(`Factory signing keys in database: ${factoryKeyCount}`);
    
    // For public endpoints like JWKS, we need to bypass Zenstack permissions
    // Use direct prisma access with careful data selection to maintain security
    logger.debug('Querying for active FACTORY signing keys');
    ...
    const signingKeys = await locals.prisma.jwtSigningKey.findMany({
      ...
      select: {
        // Only select the minimum required fields for JWKS
        keyId: true,
        publicKey: true,
        algorithm: true,
        createdAt: true
    ...
    if (!signingKeys.length) {
      logger.warn('No active factory signing keys found for JWKS endpoint');
      return json({ keys: [] });
    }
