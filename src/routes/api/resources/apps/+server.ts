...
/**
 * Unified Apps API Endpoint
 * 
 * Supports both ADMIN and USER roles.
 * - ADMIN: Sees all app resources across all accounts
 * - USER: Sees only app resources from their account memberships (via ZenStack access policies)
 * 
...
export const GET: RequestHandler = restrict(
  async ({ url, locals, auth }: { url: URL; locals: any; auth: any }) => {
    try {
      ...
      logger.error(`[ResourcesAppsAPI] Error fetching app resources: ${String(err)}`);
