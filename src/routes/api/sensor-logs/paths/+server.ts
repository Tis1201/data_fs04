...
export const GET: RequestHandler = async ({ request, url }) => {
    try {
        ...
        // 5. Build query with optional target_id filter
        ...
        console.log('[Radar API] Querying paths', {
            macAddress,
        ...
        logger.info(`[Radar API] Found ${rows.length} path records`);
        ...
        const filename = targetId
            ? `paths_${macAddress}_${targetId}_${startTimeFormatted.replace(/[ :]/g, '_')}.csv`
            : `paths_${macAddress}_${startTimeFormatted.replace(/[ :]/g, '_')}.csv`;
        ...
        logger.error('[Radar API] Error in paths endpoint', {
            error: err instanceof Error ? err.message : String(err),
            stack: err instanceof Error ? err.stack : undefined
        });
        return error(500, 'Internal Server Error: Failed to retrieve path data');
    }
};
