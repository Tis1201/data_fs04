...
/**
 * POST /api/v2/bundles/[id]/cancel
 * Cancel a deployment permanently (CANCELLED status)
 * 
 * Allowed from: IN_PROGRESS, PUBLISHED, STOPPED
 * 
 * What happens:
...
export const POST: RequestHandler = unifiedEndpoint(
	async ({ params, context }) => {
		...
		const allowed = ['IN_PROGRESS', 'PUBLISHED', 'STOPPED'];
		if (!allowed.includes(bundle.status)) {
			throw Object.assign(
				new Error(`Cannot cancel a deployment with status "${bundle.status}".`),
		...
		// 2. Update bundle status to CANCELLED
		const oldStatus = bundle.status;
		await prisma.bundle.update({
			where: { id: bundleId },
			data: { status: 'CANCELLED', updatedBy: session.user.id }
		});
		...
		try {
			...
			logger.warn(`[BundleCancel] Failed to update Redis state: ${String(e)}`);
		}
		...
		try {
			...
			logger.warn(`[BundleCancel] Failed to send MQTT notification: ${String(e)}`);
		}

		logger.info(`[BundleCancel] Bundle ${bundleId} cancelled by user ${session.user.id}. Cancelled ${pendingWaves.length} pending waves.`);

		return successResponse(
			{ bundleId, status: 'CANCELLED', cancelledWaves: pendingWaves.length },
			{ message: 'Deployment cancelled permanently.' }
		);
	},
